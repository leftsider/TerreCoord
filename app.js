...
async function generateCustomsPDF(bookingData, propertyName) {
    return new Promise((resolve, reject) => {
        ...
        // Footer
        doc.fontSize(8)
            .text('This is an automated document generated by BryandB Property Coordinator', { align: 'center', color: 'gray' });
...
async function sendEmail(to, subject, html, attachments = []) {
    ...
    try {
        await emailTransporter.sendMail({
            from: `"${process.env.EMAIL_FROM_NAME || 'BryandB'}" <${process.env.EMAIL_USER}>`,
            to,
            subject,
            html,
            attachments
        ...
        try {
            ...
            if (!isFree) {
                ...
                await sendEmail(
                    ...
                    <p>Best regards,<br>BryandB Team</p>`
                );
...
// Start server
app.listen(PORT, () => {
    console.log(`🏡 BryandB Property Coordinator running on port ${PORT}`);
